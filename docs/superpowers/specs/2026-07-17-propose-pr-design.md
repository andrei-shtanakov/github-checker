...
- `--if-match` — repeatable, optional per path: a sha256 of the base content
  ...
  `sha256(git show origin/<default>:<repo-path>)` against it; mismatch — or
  the path being absent on the default branch — yields
  `ok=False, error="base file changed; reload required"` before any branch,
  commit, or push exists. This closes the render-vs-submit race in which a
  caller could silently revert someone else's change that landed on the
  default branch in between.
...
`origin/HEAD` is not guaranteed to exist (fresh `remote add`, or the default
branch was changed on GitHub after clone). Resolution order:

1. `git symbolic-ref refs/remotes/origin/HEAD` (fast, offline);
2. `git remote show origin` (network) or `gh repo view --json defaultBranchRef`;
3. neither works → `ok=False, error="cannot determine default branch"`.

## 4. No-op semantics (decided, not accidental)

If the staged edits produce an empty diff against `origin/<default>`, the
result is `ok=False` with the **stable, machine-checkable** error string
`no changes vs <default-branch-name>` (the `no changes` prefix is part of the
contract). Rationale: "already applied" is arguably a success for a caller
like dispatcher, but `ok=True` without a `pr_url` would silently break the
existing "ok → show the PR URL" consumer pattern; and with `--if-match` in
use the stale-base case is caught earlier, making a true no-op rare. Callers
that want to treat it as success can match the documented prefix.
...
10. No-op content (equal to default) → `ok=False` with the exact
    `no changes vs <default>` marker.
