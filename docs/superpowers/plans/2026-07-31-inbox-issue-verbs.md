...
## Post-Task-6 review round: one Critical, four Important, all fail-closed class

Final whole-branch review found five real defects after Task 6 landed, verified live against `gh` 2.83.1 and fixed in one pass (no separate task numbering — this amends Tasks 3/4/5 in place):

- **C1 (Critical):** `gh search issues` has no `--limit` in the Task 3 call and defaults to 30 (confirmed live: `cli/cli` returns 30, 100 with `--limit 100`). A truncated search silently read as `ok=true, matches=[]` — a taken slug would read as free and a duplicate would get filed. Fixed by switching `issue_lookup` from `gh search issues` to `gh issue list --repo <owner>/<name> --state all --label inbox --limit 200 --json …` (no free-text term — `slug_lines` alone decides membership). `gh issue list` beats `gh search issues` on three counts confirmed live: it takes a real `--state all` (search's `--state` only accepts `open`/`closed`); it queries the API directly, not the search index, so a just-created issue is visible immediately (`issue_create` reads back right after creating); it is natively `--repo`-scoped. `--limit` still caps, so hitting it exactly (`len(candidates) >= ISSUE_LIST_LIMIT`) is now treated as possibly-truncated → `ok=False`, matches unset — never a clean or partial result. Same idiom as `pr-detail`'s `checks_truncated`/`threads_truncated`.
- **I1:** `_partition` defaulted a missing `body` to `""` (fails open — silently confirms "no claim"), while a missing `number` already hard-failed. Inverted: `item["body"]` is now indexed directly, so a candidate with no readable body fails closed like every other identity field.
- **I2:** `issue_create`'s post-create read-back collapsed "could not read", "read cleanly but found nothing", and "found a malformed claimant" into one `detail` string that lied in two of the three cases. Now distinguished into four outcomes (failed / found-nothing-yet / found-one / found-malformed / found-more-than-one), all still `ok=True, created=True` since the create itself already succeeded — the read-back is diagnostic only.
- **I3:** a non-UTF-8 `--body-file` raised `UnicodeDecodeError` (a `ValueError`, not an `OSError`) past `_run_issue_create`'s `except OSError`, reached `_dispatch_guarded`'s generic catch-all, and reported `created: null` for a request that was never attempted — un-retriable per the README's own `null` contract. Fixed at the read site: `except (OSError, UnicodeDecodeError)`, not by broadening the downstream catch-all.
- **I4:** `run_gh`'s `except FileNotFoundError` didn't cover `OSError` generally, so `issue_create`'s unbounded `--body` argv could hit `E2BIG` or `PermissionError` and raise past the "never raises" guarantee. Widened to `except (OSError, subprocess.TimeoutExpired)` — `FileNotFoundError` is already an `OSError` subclass, so this is strictly wider, not a replacement.
- **Minors folded in:** `gh issue create` now takes `--repo` too (the lookup guarding it already did); dropped `json.loads(proc.stdout or "[]")`, which laundered a genuinely empty/unreadable stdout into a confirmed empty result; `issue_create`'s pre-check no longer silently narrows several existing matches to `[0]` — it refuses (`ok=false, created=false`) with all of them in `matches`; the ambiguous `"...carry more than one slug: line"` error string became `"...contain more than one 'slug:' line"`; the dead `.replace("\r\n", "\n")` in `slug_lines` was deleted — provably redundant, since a dangling `\r` after a plain `\n` split is already whitespace, absorbed by `_SLUG_LINE_RE`'s trailing `\s*` and by `str.strip()`'s blank-line check.

Four tests were found to have survived the corresponding mutation (passed whether the bug was present or not) and were repaired in place rather than merely supplemented: the `back.ok` read-back-collapse test (`test_created_true_survives_a_failed_read_back` never actually drove `back.ok` to `False` — it used an empty-but-successful read-back, which is a different fact), the CRLF-tolerance test (didn't discriminate the now-removed dead code, but does discriminate the real `\s*` mechanism — confirmed by mutating the regex itself), the `or "[]"` fallback (no prior test exercised genuinely empty stdout), and the CLI refusal's `created` value (`test_issue_create_with_a_missing_body_file_returns_json` never asserted `created` at all). `test_a_slug_only_in_the_prose_is_not_a_match` also could not fail: its lone assertion (`matches == []`) is equally satisfied by a completely stubbed `_partition` that never appends anything — repaired to include a genuine match alongside the prose-only decoy, so only the decoy's exclusion, not blanket emptiness, makes it pass.
