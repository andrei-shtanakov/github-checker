...
stderr. **Отказ argparse — тоже JSON.** Значение флага, начинающееся с `-`
(`--slug --from`), неизвестный глагол, лишний или недостающий позиционный
раньше уходили через собственный `exit(2)` argparse с usage в stderr и вообще
без JSON. Теперь все они идут через один подкласс парсера и печатают обычный
отказной `ActionResult` с `action`/`dir`, извлечёнными из argv, и exit 1 —
машинному потребителю не нужно разбирать usage-текст, чтобы понять, что
произошло. Единственное исключение — `--help`: он идёт через `exit()`, а не
через `error()`, не является вызовом глагола и намеренно остаётся вне
контракта (exit 0, usage, без JSON). `snapshot` и интерактивный TUI в контракт
не входят — они `ActionResult` не печатают.
